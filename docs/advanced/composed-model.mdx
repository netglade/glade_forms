...
The core building block of this system is the `GladeComposedModel`, which maintains and manages a list of individual `GladeModel` instances contained within the composed form.
...
Composed models behave just like ordinary `GladeModel` objects — they can be provided using `GladeComposedModelProvider` and consumed with `GladeComposedFormBuilder`:

```dart
GladeComposedModelProvider(
  create: (context) => MyComposedModel([MyFormModel()]),
  child: GladeComposedFormBuilder<MyComposedModel>(
